...
The 0.1.7 update preserves the Linux agent runtime directory across restarts so existing service-container bind mounts keep observing the active Neptune socket. Updater 0.4.7 installs the verified executable and systemd unit within one rollback boundary and repairs stale legacy mounts. The update retains the 0.1.6 Chronos/Laboratory pipeline and state contracts. Each pipeline resolves only its required Register metadata; unrelated head credentials remain unresolved. The Linux control loop resolves Saturn coordinates once per cycle for all projects, keeping request volume independent of the number of enrolled producers. Optional Windows sync-preferences keys may be absent in an older Register; required coordinates still fail closed. Validate actual installed versions before applying an upgrade; the local compatibility fixtures cover the previous Linux 0.1.0/0.1.1 and unified 0.1.5 state formats, including partial transfer offsets, spool files, mirror settings and completed command deduplication.
