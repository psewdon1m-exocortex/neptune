...
if (args is ["version"])
{
    Console.WriteLine(typeof(Program).Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion.Split('+')[0] ?? "0.1.6-dev");
    return;
}
...
app.MapGet("/v1/projects/{projectId}/status", async (HttpContext context, string projectId, BackupWorker worker, MirrorWorker mirrorWorker, NeptuneStateStore state) =>
{
    ...
    return Results.Ok(new
    {
        ...
        version = typeof(Program).Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion.Split('+')[0] ?? "0.1.6-dev",
